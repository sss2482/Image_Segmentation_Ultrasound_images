...
name=DATA_DIR+'results_PSPNET_vgg16.xlsx'
...
for i, params in enumerate(parameters_combinations):
  back_bone = params[0]
  print(f'./best_model_PSPNET_{back_bone}_{epoch}.pth')
  ...
  if TRAINING:
      ...
      for epoch in range(0, EPOCHS):
          ...
          name=DATA_DIR+'results_PSPNET_vgg16.xlsx'
          results_df.to_excel(name)  
          torch.save(model, DATA_DIR + f'./best_model_PSPNET_{back_bone}_{epoch}.pth')
          # Save model if a better val IoU score is obtained
          if best_iou_score < valid_logs['iou_score']:
              best_iou_score = valid_logs['iou_score']
              torch.save(model, DATA_DIR + './best_model_PSPNet_vgg.pth')
              print('Model saved!')
...
if os.path.exists(DATA_DIR + './best_model_PSPNet_vgg.pth'):
    best_model = torch.load(DATA_DIR + './best_model_PSPNet_vgg.pth', map_location=DEVICE)
...
name=DATA_DIR+'results_PSPNET_vgg.xlsx'
results_df.to_excel(name)  
...
wb.save(DATA_DIR+'predictions_PSPNet_vgg.xlsx')
