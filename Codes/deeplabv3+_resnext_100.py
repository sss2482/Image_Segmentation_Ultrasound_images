...
name=DATA_DIR+'results_DeepLabV3Plus_resnext50_32x4d.xlsx'
...
for i, params in enumerate(parameters_combinations):
  ...
  if os.path.exists(DATA_DIR + './best_model_DeepLabV3Plus_resnext50_32x4d.pth'):
    best_model = torch.load(DATA_DIR + './best_model_DeepLabV3Plus_resnext50_32x4d.pth', map_location=DEVICE)
  ...
  if TRAINING:
      ...
      for epoch in range(0, EPOCHS):
          ...
          rn=len(results_df.index)
          print(rn)
          results_df.loc[rn] = [*params, epoch, train_logs['iou_score'], train_logs['dice_loss'] ]
          name=DATA_DIR+'results_DeepLabV3Plus_resnext50_32x4d.xlsx'
          results_df.to_excel(name)  
          torch.save(model, DATA_DIR + f'./best_model_DeepLabV3Plus_{back_bone}_{epoch}.pth')
          # Save model if a better val IoU score is obtained
          if best_iou_score < valid_logs['iou_score']:
              best_iou_score = valid_logs['iou_score']
              torch.save(model, DATA_DIR + './best_model_DeepLabV3Plus_resnext50_32x4d.pth')
              print('Model saved!')
...
wb.save(DATA_DIR+'predictions_DeepLabV3Plus_resnext.xlsx')
