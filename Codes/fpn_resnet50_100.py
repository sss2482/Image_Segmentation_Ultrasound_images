...
name=DATA_DIR+'results_FPN_resnet50_m.xlsx'
...
for i, params in enumerate(parameters_combinations):
  ...
  if TRAINING:
      ...
      for epoch in range(0, EPOCHS):
          ...
          name=DATA_DIR+'results_FPN_resnet50_m.xlsx'
          results_df.to_excel(name)  
          torch.save(model, DATA_DIR + f'./best_model_FPN_{back_bone}_{epoch}.pth')
          # Save model if a better val IoU score is obtained
          if best_iou_score < valid_logs['iou_score']:
              best_iou_score = valid_logs['iou_score']
              torch.save(model, DATA_DIR + './best_model_FPN_resnet50.pth')
              print('Model saved!')
...
if os.path.exists(DATA_DIR + './best_model_FPN_resnet50.pth'):
    best_model = torch.load(DATA_DIR + './best_model_FPN_resnet50.pth', map_location=DEVICE)
...
wb.save(DATA_DIR+'predictions_FPN_resnet50.xlsx')
